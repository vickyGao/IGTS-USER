rootApp.controller('rootController', function ($scope) {
    $scope.$on('event:ShowCreateCoverModalRequest', function (event, commodityPostBody) {
        $scope.$broadcast('event:ShowCreateCoverModal', commodityPostBody);
    });
    $scope.$on('event:ShowWarnMessageRequest', function (event, message) {
        $scope.$broadcast('event:ShowWarnMessage', message);
    });
...
rootApp.controller('publishController', function ($scope, $cookieStore, Upload, TagService, ImageService) {
    TagService.listDetail().success(function (data) {
        $scope.tagArray = data.tags;
    });
    $scope.division  = addressChina;

    $scope.$on('event:ShowWarnMessage', function (event, message) {
        $scope.publish_warn_message = message;
    });


	$scope.coverImages = null;
	ImageService.getAll().success(function (data) {
        $scope.coverImages = data.images;
    });

    $scope.doSubmit = function () {
        if($scope.commodity.title ){
            if($scope.commodity.title.length < 20){
                $scope.publish_warn_message = "商品标题内容太短！";
            }else if($scope.commodity.title.length > 43){
                $scope.publish_warn_message = "商品标题内容太多！";
            } else if($scope.commodity.price == null ||
                    $scope.commodity.carriage == null ||
                    $scope.commodity.tags == null || $scope.commodity.tags.id == null ||
                    $scope.commodity.district == null){
                        $scope.publish_warn_message = null;
            }else  if($scope.coverImages == null || $scope.coverImages.length == 0){
                $scope.publish_warn_message = "您的相册暂无图片，请先上传！";
            } else{
                 var commodityAddress = null;
                 if($scope.commodity.district){
                     commodityAddress = $scope.commodity.district + " ";
                     if($scope.address.addresscity){
                        commodityAddress += $scope.address.addresscity + " ";
                        if($scope.address.addressdistrict){
                            commodityAddress += $scope.address.addressdistrict;
                         }
                     }
                 }

                     var commodityTags = new Array();
                     if($scope.commodity.tags){
                         var commodityParentTag = {
                                 'id' : $scope.commodity.tags.id
                         };
                         commodityTags.push(commodityParentTag);

                         if($scope.commodity.tags.childtag){
                             var commodityChildTag = {
                                    'id' : $scope.commodity.tags.childtag.id
                            };
                             commodityTags.push(commodityChildTag);
                             viewCommodityChildTag = $scope.commodity.tags.childtag;
                        }
                     }

                   // $scope.commodity.tags = commodityTags;
                    var postBody = {
                        'commodity': {
                            'title' : $scope.commodity.title,
                            'carriage' : $scope.commodity.carriage,
                            'price' : $scope.commodity.price,
                            'district' : commodityAddress,
                            'description' : $scope.commodity.description,
                            'tags' : commodityTags
                        }
                    };

                   $scope.publish_warn_message = null;
                   $scope.$emit('event:ShowCreateCoverModalRequest', postBody);
            }
        }
    }
...
rootApp.controller('CreateCoverModalController', function ($scope, ImageService, CoverService, CommodityService) {
    var createCommodity = null;
    $scope.$on('event:ShowCreateCoverModal', function (event, commodityPostBody) {
        createCommodity = commodityPostBody;

        ImageService.getAll().success(function (data) {
            $scope.images = data.images;
            /*if(data.images.length == 0){
                $scope.$emit('event:ShowWarnMessageRequest', "您的相册暂无图片，请先上传！");
            }else{*/
            //}
    ...
    $scope.doSubmit = function (selectedImages) {
         CommodityService.create(createCommodity).success(function (data) {
            var postBody = {
                        images:selectedImages
                    };
            CoverService.create(data.commodity.id, postBody)
